...
fn main() {
    ...
    println!("  Input: num_inputs=0, feeds rest · Hidden: combines → transforms → passes on · Output: its tensor = network output\n");
    ...
    let both = Node::new_hidden(0, 2, 2).with_hidden_dim(16).with_activation(Activation::ReLU);
    println!("  with_activation(GeLU): act = {:<8} dim = {:?} combine = {:?}", gelu.activation, gelu.hidden_dim, gelu.combine_op);
    println!("  with_hidden_dim(32):   act = {:<8} dim = {:?} combine = {:?}", wide.activation, wide.hidden_dim, wide.combine_op);
    println!("  with_combine_op(Mean): act = {:<8} dim = {:?} combine = {:?}", mean.activation, mean.hidden_dim, mean.combine_op);
    println!("  chained both:          act = {:<8} dim = {:?} combine = {:?}", both.activation, both.hidden_dim, both.combine_op);
    println!("  (each builder touches only its own field — order doesn't matter)\n");

    // ── 3. Activations ──────────────────────────────────────────────────────
    println!("═ 3. Activations — all 8 variants ═");
    let x = Variable::new(Tensor::from_f32(&[-1.0, 0.5, 2.0], &[3], Device::CPU).unwrap(), false);
    for act in [Activation::Identity, Activation::ReLU, Activation::GeLU, Activation::SiLU,
                Activation::SELU, Activation::Tanh, Activation::Sigmoid, Activation::Mish] {
        let y = act.apply(&x).unwrap();
        println!("  {:<10} {:>12?} -> {:?}", act, x.data().to_f32_vec().unwrap(), y.data().to_f32_vec().unwrap());
    }
    ...
    g.nodes.push(Node::new_hidden(1, 2, 1).with_activation(Activation::SELU));
    g.nodes.push(Node::new_output(2, 1, 1));
    g.connections.push(Connection { from: Port { node: 0, index: 0 }, to: Port { node: 1, index: 0 } });
    g.connections.push(Connection { from: Port { node: 0, index: 1 }, to: Port { node: 1, index: 1 } });
    g.connections.push(Connection { from: Port { node: 1, index: 0 }, to: Port { node: 2, index: 0 } });
    g.refresh_labels();
    g.validate().unwrap();
    println!("  validates ✓ ({} wires):", g.connection_labels().len());
    for (from, to) in g.connection_labels() { println!("    {from} -> {to}"); }
    let net = Network::build(&g, Device::CPU).unwrap();
    let input = Variable::new(Tensor::randn(&[2, g.options.input_dim as i64],
        TensorOptions { dtype: DType::Float32, device: Device::CPU }).unwrap(), false);
