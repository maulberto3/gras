...
fn main() {
    ...
    let (run_dir, net_hash, data_dir) = if args.len() >= 3 {
        (PathBuf::from(&args[1]), args[2].clone(), PathBuf::from("data/mnist/train"))
    } else {
    ...
    let trainer = TabularTrainer::new(loss_fn).with_learning_rate(1e-3).with_grad_clip(1.0);
    ...
    for step in 0..net_state.step {
        let batch = stream.train_batch(&dataset, step as u64).expect("Failed to load train batch");
        gras::utils::race_steps::seed_step_randomness(net_state.net_seed as u64, step as u64, 0);
        let train_loss = gras::utils::race_steps::train_one_step(&mut net, optimizer.as_mut(), &loss_fn, &batch, 1.0).unwrap();
        
        if step % 2 == 0 || step == net_state.step - 1 {
            let eval_batch = stream.eval_batch(&dataset, step as u64).expect("Failed to load eval batch");
            let report = gras::utils::race_steps::eval_one_step(&mut net, &loss_fn, &fitness, &[], &eval_batch).unwrap();
            println!(
                "Replay step {:3} | Train Loss: {:.4} | Eval Loss: {:.4} | Fitness: {:.4}",
                step, train_loss, report.eval_loss.unwrap(), report.fitness
            );
        }
    }
    ...
    if let Some(metrics) = &net_state.last_metrics {
        ...
        println!("  Recorded Eval Loss: {:.4}", metrics.eval_loss.unwrap_or(0.0));
    }
