...
/// Markdown rendering of a [`Topology`]: header, nodes table, wires table,
/// and the visual wiring diagram in a code block.
pub(crate) fn topology_markdown(graph: &Topology, fitness: Option<f64>) -> String {
    let mut out = String::new();
    ...
    // Summary
    out.push_str(&format!(
        "**{} nodes** · **{} wires** · combine: `{:?}` · input {} → hidden {} → output {}\n\n",
        graph.nodes.len(),
        graph.connections.len(),
        graph.options.combine_op,
        graph.options.input_dim,
        graph.options.hidden_dim,
        graph.options.output_dim,
    ));

    // Nodes table
    out.push_str("## Nodes\n\n");
    out.push_str("| ID | Kind | In | Out | Activation | Combine | Dims |\n");
    out.push_str("|----|------|----|-----|------------|---------|------|\n");
    for node in &graph.nodes {
        ...
        let combine = match node.combine_op {
            Some(op) => format!("{op:?}"),
            None => "—".into(),
        };
        let dims = node.hidden_dim.map_or("—".into(), |d| {
            format!("{}→{d}", graph.options.hidden_dim)
        });
        out.push_str(&format!(
            "| {id} | {kind} | {ni} | {no} | {act} | {combine} | {dims} |\n",
            id = node.id,
            ni = node.num_inputs,
            no = node.num_outputs,
        ));
    ...
    for (i, conn) in graph.connections.iter().enumerate() {
        out.push_str(&format!(
            "| {} | n{}_o{} | n{}_i{} |\n",
            i, conn.from.node, conn.from.index, conn.to.node, conn.to.index,
        ));
    }
    out.push('\n');
...
/// Markdown rendering of a [`Network`]: layers table with dims, activations,
/// and source wiring.
pub(crate) fn network_markdown(g: &Network) -> String {
    let mut out = String::new();

    out.push_str("## Network layers\n\n");
    out.push_str(&format!(
        "**{} nodes** · **{} wires** · **{} param tensors** · combine: `{:?}`\n\n",
        g.layers.len(),
        g.connections.len(),
        g.parameters().len(),
        g.combine_op,
    ...
    out.push_str("| Layer | Kind | Linear | Activation | Sources |\n");
    out.push_str("|-------|------|--------|------------|---------|\n");
    ...
    // Per-node layers
    let node_sources = crate::topology::build_node_sources(&g.connections, &g.nodes.iter().map(|n| n.num_inputs).collect::<Vec<_>>());
    for (i, node) in g.nodes.iter().enumerate() {
        ...
        let (in_dim, out_dim) = g.node_dims[i];
        let marker = if i == g.output_node { " 🏁" } else { "" };
        ...
        out.push_str(&format!(
            "| n{i} {kind}{marker} | {kind} | {in_dim} → {out_dim} | {} | {src_str} |",
            node.activation,
        ));
        out.push('\n');
    }
