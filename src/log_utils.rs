...
pub(crate) fn log_run_start(options: &EngineOptions, run_dir: &Path, direction: Direction) {
    ...
    log::info!("  fitness {:?} ({better} = better)", options.fitness);
...
pub(crate) fn log_run_summary(
    ...
    log::info!("  duration    {run_elapsed:.2?}");
    log::info!("  run_seed    {run_seed}  (reproduce with .set_seed(Some({run_seed})))");
    ...
    log::info!("  fitness     {:?} ({})", options.fitness,
    ...
    log::info!("  topo opts   input {} → hidden {} → output {} · combine {:?}",
        options.topology.input_dim, options.topology.hidden_dim,
        options.topology.output_dim, options.topology.combine_op);
    log::info!("  GP pools    hidden {:?} · combine {:?}",
        options.hidden_dim_pool, options.combine_op_pool);
...
fn log_winner(best: &BestIndividual) -> Result<(), EngineError> {
    ...
    if !dims.is_empty() {
        ...
        log::info!("  winner      node dims  [{}]", dim_str.join(", "));
    }
