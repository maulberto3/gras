...
#[derive(Debug)]    pub struct RunConfig {
        pub run_id: String,
        pub run_seed: u64,
        pub fitness_label: FitnessLabel,
        pub fitness_direction: Direction,
...
impl RunHeader {
    ...
    pub fn from_race_options(cfg: RunConfig) -> Self {
        ...
        RunHeader {
            ...
            output_dim,
            topology_options,
            hidden_dim_pool: format!(
                "{}..={}",
                hidden_dim_pool.start(),
                hidden_dim_pool.end()
            ),
            hidden_dim_stride,
            combine_op_pool,
            activation_pool,
            standardize_op_pool,
            informative_metrics: informative_metrics.into_iter().map(|m| m.label().to_string()).collect(),
            max_steps,
            grace_steps,
            divergence_threshold,
            started_at: None,
    ...
    pub fn from_json(raw: &str) -> Result<Self> {
        ...
        let fitness_direction = v.get("fitness_direction")
            .and_then(|f| f.as_str())
            .and_then(|s| match s {
        ...
        let fitness_label = serde_json::from_value(v.get("fitness_label").cloned().unwrap_or_else(|| serde_json::Value::String("loss".into()))).unwrap_or(FitnessLabel("loss".into()));
        ...
        let topology_options = v.get("topology_options")
            .map(|t| serde_json::from_value(t.clone()).unwrap_or_default())
            .unwrap_or_default();
        let hidden_dim_pool = v.get("hidden_dim_pool")
            .and_then(|f| f.as_str())
            .map(|s| s.to_string())
            .unwrap_or_else(|| "4..=8".into());
        let hidden_dim_stride: usize = v.get("hidden_dim_stride")
            .and_then(|f| f.as_u64())
            .map(|n| n as usize)
            .unwrap_or(1);
        let combine_op_pool: Vec<String> = serde_json::from_value(v.get("combine_op_pool").cloned().unwrap_or(serde_json::Value::Null)).unwrap_or_default();
        let activation_pool: Vec<String> = serde_json::from_value(v.get("activation_pool").cloned().unwrap_or(serde_json::Value::Null)).unwrap_or_default();
        let standardize_op_pool: Vec<String> = serde_json::from_value(v.get("standardize_op_pool").cloned().unwrap_or(serde_json::Value::Null)).unwrap_or_default();
        let informative_metrics: Vec<String> = serde_json::from_value(v.get("informative_metrics").cloned().unwrap_or(serde_json::Value::Null)).unwrap_or_default();
        let max_steps = v.get("max_steps").and_then(|f| f.as_u64()).map(|n| n as usize);
        let grace_steps = v.get("grace_steps").and_then(|f| f.as_u64()).map(|n| n as usize);
        let divergence_threshold = v.get("divergence_threshold").and_then(|f| f.as_f64()).map(|n| n as f32);
        let started_at = v.get("started_at").and_then(|f| f.as_str()).map(|s| s.to_string());
        let train_eval_split_ratio = v.get("train_eval_split_ratio").and_then(|f| f.as_f64()).map(|n| n as f32);
        let held_out_eval_rows = v.get("held_out_eval_rows").and_then(|f| f.as_u64()).map(|n| n as usize);
        Ok(RunHeader {
            run_id: v.get("run_id").and_then(|f| f.as_str()).unwrap_or("unknown").to_string(),
...
impl NetState {
    ...
    pub fn stamp_meta(
        ...
        // learning_rate/grad_clip moved to the user's Trainer (training is no
        // longer engine-owned); recorded as None in the meta ledger.
    ...
    pub fn new(topo: &Topology, step: usize, created_from: Option<String>) -> Result<Self> {
        let topo_json = topo.to_json().map_err(|e| {
            EngineError::Json(format!("net state topology json: {e}"))
        })?;
    ...
    pub fn to_json(&self) -> Result<String> {
        ...
        if to_write.meta.params.is_none() {
            let topo = crate::graph::topology::Topology::from_json(&to_write.topology).map_err(|e| EngineError::Json(format!("net state meta backfill: {e}")))?;
            let net = crate::graph::network::Network::build(&topo, crate::Device::CPU).map_err(|e| EngineError::Json(format!("net state meta backfill build: {e}")))?;
    ...
    pub fn from_json(raw: &str) -> Result<Self> {
        Ok(serde_json::from_str(raw).map_err(|e| {
            EngineError::Json(format!("net state parse: {e}"))
        })?)
    }
    ...
    pub fn topology(&self) -> Result<Topology> {
        Ok(Topology::from_json(&self.topology).map_err(|e| {
            EngineError::Json(format!("net state topology parse: {e}"))
        })?)
    }
}
...
impl RaceState {
    ...
    pub fn insert(&mut self, state: NetState, initial_fitness: Option<f32>) {
        ...
        if let Some(entry) = self.topology_lifetime.get_mut(&key) {
            ...
            self.topology_lifetime
                .insert(key.clone(), TopologyLifetime::new(state.entered_at_step, initial_fitness));
        }
    ...
    pub fn record_step(
        ...
        let state = self
            .nets
            .get_mut(hash)
            .ok_or_else(|| EngineError::InvalidOptions(format!("race state: net {hash} not live")))?;
        state.record_metrics(metrics.clone());
        state.advance_step();
        let fitness = metrics.fitness;
        let fitness = if fitness.is_finite() { Some(fitness) } else { None };
        let entry = self
            .topology_lifetime
...
mod tests {
    ...
    fn fresh_dir() -> PathBuf {
        let dir = std::env::temp_dir().join(format!("gras_race_state_test_{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos()));
    ...
    fn baseline_header(run_id: &str) -> RunHeader {
        RunHeader::from_race_options(RunConfig {
            ...
            grace_steps: Some(20),
            divergence_threshold: Some(0.20),
    ...
    fn tiny_topology() -> Topology {
        let mut topo = crate::graph::topology::Topology::new(0, Some(crate::graph::topology::TopologyOptions {
            topology_seed: 7,
        ...
        topo.nodes.push(crate::graph::node::Node::new_hidden(1, 2, 4));
        topo.nodes.push(crate::graph::node::Node::new_output(2, 4, 2));
        topo
    }
    ...
    fn engine_json_write_and_read_back_identical() {
        ...
        assert_eq!(loaded.grace_steps, Some(20));
        assert_eq!(loaded.divergence_threshold, Some(0.20));
    ...
    fn net_state_round_trip_keeps_hash_and_seed_and_step() {
        ...
        assert_eq!(
            loaded.created_from.as_deref(),
            Some("parent-a x parent-b")
        );
    ...
    fn race_state_lifetime_tracks_appearances_and_span() {
        ...
        st.record_step(&h, NetMetrics { step: 10, train_loss: 0.0, eval_loss: None, fitness: 0.4, informative: vec![] }).unwrap();
        st.record_step(&h, NetMetrics { step: 11, train_loss: 0.0, eval_loss: None, fitness: 0.5, informative: vec![] }).unwrap();
    ...
    fn removed_net_file_is_gone() {
        ...
        let state = NetState::new(&topo, 0, None).unwrap();
        write_net_state(&dir, &state).unwrap();
        assert!(dir.join("nets").join(format!("{}.json", state.hash)).exists());
